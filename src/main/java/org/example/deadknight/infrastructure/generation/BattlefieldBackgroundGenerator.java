...
/**
 * Генератор «поля брани» (фоновой карты) для FXGL на основе тайлов.
 *
 * <p>Работает поверх JavaFX {@link Pane}: выкладывает плитки (пол) и декоративные объекты (деревья).
 * Подразумевается фиксированный размер тайла {@link #tileSize} в пикселях и прямоугольная сетка {@code tilesX × tilesY}.
 *
 * <h2>Производительность</h2>
 * <ul>
 *   <li>Не генерируйте чрезмерно большие карты (например, 256×256 при {@code tileSize=128})
 *       — это десятки тысяч узлов в сцене и гигабайты текстур в памяти.</li>
 *   <li>Рассмотрите использование отрисовки в один {@code Canvas}, чанкинг/ленивую подгрузку или
 *       объединение спрайтов (sprite atlas) с параллакс-слоями.</li>
 * </ul>
 *
 * @implNote Методика проста: для каждого тайла создаётся {@link ImageView} из ассета FXGL и добавляется в корневую панель.
 *           Для больших миров предпочтительнее рисовать на {@code Canvas} или в текстуру (snapshot) батчем.
 * @since 1.0
 */
public class BattlefieldBackgroundGenerator {

    /**
     * Размер тайла в пикселях. Сделан {@code volatile}, чтобы изменения были видны из других потоков,
     * если генерация запускается асинхронно (хотя обычно следует настраивать перед генерацией).
     */
    ...
    /** Источник случайности для повторяемой генерации. */
    private final Random rnd;


    /**
     * Пути к вариантам плитки «камня/травы» (нижний слой).
     * <p>FXGL загрузит изображения по указанным путям относительно каталога ресурсов.</p>
     */
    ...
    /**
     * Пути к вариантам деревьев (верхний декоративный слой).
     */
    ...
    /**
     * Создаёт генератор для указанного размера сетки.
     *
     * @param tilesX количество тайлов по оси X
     * @param tilesY количество тайлов по оси Y
     * @param seed   зерно для {@link Random} — одинаковое зерно даёт одинаковый результат
     */
    ...
    /**
     * Генерирует нижний слой — «пол» (камень/трава) — и добавляет его на указанный {@link Pane}.
     *
     * <p>Для разнообразия используется смещённая вероятность выбора первого варианта
     * (см. {@link #pick(String[], double)} с {@code biasKeep=0.85}).</p>
     *
     * @param root корневая панель JavaFX, куда будут добавлены {@link ImageView} тайлов
     */
     ...
     * @return выбранная строка с путём к ассету
     */
    ...
    /**
     * Выбирает случайный элемент массива с перекосом в пользу первого.
     *
     * @param arr      массив кандидатов
     * @param biasKeep вероятность выбрать элемент с индексом 0 (например, 0.85 означает «в 85% случаев берём arr[0]»)
     * @return выбранная строка с путём к ассету
     */
    ...
    /**
     * Создаёт {@link ImageView} для ассета FXGL и добавляет его на {@code root} по координатам {@code (px, py)}.
     *
     * <p>Масштаб задаёт габариты в единицах тайла: {@code 1.0} означает «ровно размер тайла»,
     * {@code 2.0} — «в два раза больше» и т. п.</p>
     *
     * @param asset путь до текстуры (относительно ресурсов FXGL)
     * @param px    позиция по X в пикселях
     * @param py    позиция по Y в пикселях
     * @param scale масштаб относительно {@link #tileSize}
     * @param root  панель, куда добавить созданный узел
     */
